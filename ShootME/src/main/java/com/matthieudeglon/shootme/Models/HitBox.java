...
public class HitBox {
    ...
    public HitBox(double top, double left, double width, double height) {
        box = new double[]{top, left, top + height, left + width};
    }


    public void compute_tiles_bounds(GameMapModel map) {
    ...
    protected boolean isOutOfMap(GameMapModel M) {
    ...
    private boolean check_bottom_and_right(GameMapModel M) {
        return get_right_box() >= M.get_width() || get_bottom_box() >= M.get_height();
    }

    protected boolean checkIfObjectCanMoveOnNeighboursTiles(GameMapModel M, DynamicObject D) {
